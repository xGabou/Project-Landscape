...
public final class StageBaseline {
    public static void run(ServerLevel level, Evidence out) throws Exception {
        ...
        out.row("stage_identity","schemaVersion",1,"source","203dc3c1c2e1d4e09fa911a3d6dc7472664f4ba7",
            "backend","LEGACY_RTF_V0","fixtureVersion",1,"configurationFingerprint",fixture.get("configurationFingerprint").getAsString(),
        ...
        for(var seedFixture:fixture.getAsJsonArray("seedFixtures")) {
            ...
            try {
                var h=context.generator.getHeightmap();int ordinal=0;
                for(var value:sf.getAsJsonArray("points")) {
                    ...
                    cell.terrain=TerrainType.FLATS;cell.beachNoise=h.beachNoise().compute(p.x(),p.z(),0);
                    h.continent().apply(cell,p.x(),p.z());
                    capture(out,seed,ordinal,p,"continent",cell,h);
                    h.region().apply(cell,p.x(),p.z());h.terrain().apply(cell,p.x()*h.terrainFrequency(),p.z()*h.terrainFrequency());
                    capture(out,seed,ordinal,p,"terrain",cell,h);
                    h.applyRivers(cell,p.x(),p.z(),h.continent().getRivermap(cell));
                    capture(out,seed,ordinal,p,"hydrology",cell,h);
                    h.applyClimate(cell,p.x(),p.z(),true);
