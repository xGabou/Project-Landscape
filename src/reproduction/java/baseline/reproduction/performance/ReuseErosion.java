/* Offline candidate derived from ReTerraForged, Copyright (c) 2023 ReTerraForged, MIT License. See LICENSE. */
...
public class ReuseErosion implements Filter {
    ...
    public ReuseErosion(final int seed, final int mapSize, final FilterSettings.Erosion settings, final Modifier modifier) {
        this.seed = seed;
    ...
    public void apply(Filterable map, int regionX, int regionZ, int iterationsPerChunk) {
        ...
        final float[] strengths = new float[cells.length];
        for(int index=0;index<cells.length;index++) {
            Cell cell=cells[index];float strength=1.0F;
            float erosion=cell.terrain.erosionModifier();
            if(erosion!=1.0F){float alpha=NoiseUtil.map(cell.terrainRegionEdge,0.0F,0.15F,0.15F);strength=NoiseUtil.lerp(1.0F,erosion,alpha);}
            if(cell.riverMask<0.1F)strength*=NoiseUtil.map(cell.riverMask,0.002F,0.1F,0.098F);
    ...
    private void deposit(final Cell cell, final float strength, final float amount) {
        if (!cell.erosionMask) {
            final float change = this.modifier.getValueModifier(cell.height) * strength * amount;
            cell.height += change;
            cell.sediment += change;
        }
    }
    
    private void erode(final Cell cell, final float strength, final float amount) {
        if (!cell.erosionMask) {
            final float change = this.modifier.getValueModifier(cell.height) * strength * amount;
            cell.height -= change;
    ...
    public static IntFunction<ReuseErosion> factory(final GeneratorContext context) {
    ...
    private static class Factory implements IntFunction<ReuseErosion> {
        ...
        @Override
        public ReuseErosion apply(final int size) {
            return new ReuseErosion(this.seed, size, this.settings, this.modifier);
        }
    }
}
