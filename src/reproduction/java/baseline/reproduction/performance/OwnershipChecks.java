...
final class OwnershipChecks {
    static void run(GeneratorContext context,Path out)throws Exception {
        ...
        var content=new ManifestContent(GenerationVersions.planned(),8675309,net.minecraft.world.level.Level.OVERWORLD.location(),
                ...
                Map.of("offline-ownership-fixture-only",GenerationFingerprint.bytes(new byte[]{1})),Optional.of(new com.gabou.projectlandscape.biome.VanillaBiomeCatalog().fingerprint()));
        ...
        var reference=new com.gabou.projectlandscape.climate.BaselineClimateModel(generation.seeds(),config)
                .sample(com.gabou.projectlandscape.climate.ClimateGeographyAdapters.of(g,g.macroProvider(),g.distanceField()),0,0);
        if(!baseline.equals(reference))throw new AssertionError("Service changes climate");
        var sources=System.getProperty("task6b.sourceChecks","false").equals("true")?SourceReuseChecks.run(context,generation,out):List.<com.gabou.projectlandscape.biome.ClimateBiomeSource>of();
