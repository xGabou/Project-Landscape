...
final class IslandExperiment {
    ...
    static void run(Path out)throws Exception {
        ...
        var known=new HashMap<Long,List<IslandModel.Island>>();var reference=original(seeds,settings);
        ...
        try(var recording=new jdk.jfr.Recording(jdk.jfr.Configuration.getConfiguration("profile"))){
            ...
            for(int workers:new int[]{1,8,24,48})for(String workload:List.of("hot","cold","mixed95_5","trace"))for(int pass=-1;pass<3;pass++) {
                for(int order=0;order<2;order++) {
                    ...
                    var model=original(seeds,settings);var production=new PaMacroGeography(seeds,settings).islands();Access access=candidate?production::islands:model::islands;
                    ...
                    rows.add(Map.of("workers",workers,"workload",workload,"pass",pass,"candidate",candidate,"queries",count,"wallNanos",nanos,"cpuNanos",cpu,"blockedCount",blocked,"blockedThreadMillis",blockedMs,"frontHits",production.cacheStats().get("frontHits")));
                    production.close();if(production.cacheStats().get("frontEntries")!=0||production.cacheStats().get("entries")!=0)throw new AssertionError("Clear retained cache");
                    try{production.islands(hot.get(0));throw new AssertionError("Closed query accepted");}catch(IllegalStateException expected){}
                }
            }
    ...
    private static ReferenceIslandModel original(GenerationSeedService seeds,MacroGeographySettings settings){var sites=new MacroSiteField(seeds,settings);return new ReferenceIslandModel(seeds,settings,new ReservedCellContinentModel(sites,new CoastlineDetail(seeds,settings.coastlineDetailBlocks()),settings));}
}
