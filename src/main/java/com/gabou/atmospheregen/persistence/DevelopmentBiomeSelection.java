...
public final class DevelopmentBiomeSelection {
    ...
    public static Optional<GenerationManifest> requested(Path directory,ServerLevel level,ChunkGenerator generator,GeneratorContext terrain){Path file=path(directory);if(!Files.exists(file))return Optional.empty();try{
        var json=JsonParser.parseString(Files.readString(file));var climate=BaselineClimateConfig.Planned.CODEC.parse(JsonOps.INSTANCE,json.getAsJsonObject().get("climate")).getOrThrow(false,s->{});var resolver=BiomeResolverConfig.Planned.CODEC.parse(JsonOps.INSTANCE,json.getAsJsonObject().get("resolver")).getOrThrow(false,s->{});var macro=MacroGeographySettings.defaults();var data=new TreeMap<>(LegacyGenerationData.capture(level,generator));if(raccoonman.reterraforged.world.worldgen.terrablender.TBCompat.isEnabled())data.put("registry/minecraft:worldgen/noise_settings",V1TerraBlenderData.noiseSettings(level.registryAccess()));var retained=LegacyPresetSnapshot.capture(terrain.preset);data.put("retained_terrain_effective_preset",GenerationFingerprint.of(retained.effectivePreset()));data.put("retained_tile_geometry",GenerationFingerprint.of(CanonicalJson.of(new com.google.gson.JsonPrimitive("tile=3;border="+retained.borderChunks()))));var settings=new PlannedGeographySettings(macro.continentScaleBlocks(),macro.minimumMajorOceanWidthBlocks(),terrain.preset.terrain().general.terrainRegionSize,1000,0,Optional.of(macro));var catalog=new VanillaBiomeCatalog();return Optional.of(GenerationManifest.create(new ManifestContent(GenerationVersions.planned(),level.getSeed(),level.dimension().location(),new WorldGeographyConfig(Optional.empty(),Optional.of(settings)),new BaselineClimateConfig(Optional.of(climate)),new BiomeResolverConfig(Optional.of(resolver)),data,Optional.of(catalog.fingerprint()))));
