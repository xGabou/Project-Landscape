...
$forbidden=@($geography | Select-String -Pattern 'com\.gabou\.atmospheregen\.api\.(climate|biome)|raccoonman\.|BaselineClimateProvider|ClimateBiomeResolver' | Where-Object {$_.Line.TrimStart() -match '^import '})
...
if($api | Select-String -Pattern '^import raccoonman\.') {throw 'Public API exports inherited implementation'}
$stages=@('LegacyContinentStage','LegacyTerrainStage','LegacyHydrologyStage','LegacyGeographyFinalizationStage')
foreach($stage in $stages){if(Select-String "src/main/java/raccoonman/reterraforged/world/worldgen/cell/geography/$stage.java" -Pattern '^import .*\.(climate|biome)\.') {throw "$stage imports climate/biome authority"}}
$compat=@(Get-ChildItem src/main/java/raccoonman/reterraforged/world/worldgen/cell/geography -Filter '*.java' | Select-String -Pattern '^import .*\.(climate|biome)\.' | ForEach-Object { [ordered]@{file=$_.Filename;import=$_.Line.Trim()} })
